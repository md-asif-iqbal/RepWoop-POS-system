...
export default function SummaryReport() {
  ...
  const resetFilter = () => { setStartDate(""); setEndDate(""); setFiltered(false); };
  ...
  const renderTable = ({ title, titleIcon: TIcon, titleColor, data, columns, search, setSearch, searchField, page, setPage, perPage, setPerPage, csvFile, excelName, pdfName, totalKey, totalSpan, useDateFilter }) => {
    const dateFiltered = useDateFilter ? filterByDate(data) : data;
    const searchFiltered = dateFiltered.filter((item) => String(item[searchField] || "").toLowerCase().includes(search.toLowerCase()));
    const rows = paginate(searchFiltered, page, perPage);
    const totalPages = Math.ceil(searchFiltered.length / perPage) || 1;
    ...
    return (
      <Card className="overflow-hidden border-0 shadow-lg">
        <div className={`bg-gradient-to-r ${titleColor} px-5 py-3.5 flex items-center gap-2.5`}>
          ...
          <Badge className="ml-auto bg-white/20 text-white hover:bg-white/30 text-xs">{searchFiltered.length} entries</Badge>
        </div>
        <CardContent className="p-4 space-y-4">
          <div className="flex flex-col sm:flex-row justify-between gap-3">
            <div className="flex flex-wrap gap-2">
              <Button size="sm" variant="outline" className="text-emerald-600 border-emerald-200 hover:bg-emerald-50 h-8 text-xs" onClick={() => exportToExcel(dateFiltered, excelName)}>
                <FileSpreadsheet className="h-3.5 w-3.5 mr-1.5" />Excel
              </Button>
              <Button size="sm" variant="outline" className="text-blue-600 border-blue-200 hover:bg-blue-50 h-8 text-xs">
                <CSVLink data={dateFiltered} filename={csvFile} className="flex items-center gap-1.5">
                  <FileText className="h-3.5 w-3.5" />CSV
                </CSVLink>
              </Button>
              <Button size="sm" variant="outline" className="text-red-600 border-red-200 hover:bg-red-50 h-8 text-xs" onClick={() => exportPDF(dateFiltered, pdfName)}>
                ...
                <tfoot>
                  <tr className="bg-gradient-to-r from-indigo-50 to-purple-50 font-semibold">
                    ...
                    <td className="px-3 py-2.5 text-xs text-indigo-900">
                      <Badge variant="default" className="font-bold">TK {calcTotal(dateFiltered, totalKey).toLocaleString()}</Badge>
                    </td>
                  </tr>
                </tfoot>
              )}
      ...
      <div className="p-4 sm:p-6 mt-[25%] sm:mt-[5%] w-full max-w-[1400px] mx-auto space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 mt-0.5">Complete business summary with date filtering</p>
            </div>
          </div>
        </div>
        ...
        <Card className="border-0 shadow-md">
          <CardContent className="p-4">
            <div className="flex items-center gap-2 mb-3">
              ...
              {filtered && (
                <Badge variant="info" className="ml-2 text-xs">Active</Badge>
              )}
            </div>
            <div className="flex flex-col sm:flex-row gap-3">
              ...
              <div className="flex gap-2 items-end">
                <Button onClick={handleFilter} size="sm" className="h-9 bg-gradient-to-r from-indigo-500 to-purple-600 hover:from-indigo-600 hover:to-purple-700">
                  <Filter className="h-3.5 w-3.5 mr-1.5" />Filter
                </Button>
                <Button onClick={resetFilter} size="sm" variant="outline" className="h-9 text-gray-600">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {renderTable({
            ...
            data: topSaleProducts, useDateFilter: true,
            columns: [
              { key: "sl", label: "SL" },
          ...
          {renderTable({
            ...
            data: expenses, useDateFilter: true,
            columns: [
              { key: "sl", label: "SL" },
          ...
          {renderTable({
            ...
            data: paymentsToSuppliers, useDateFilter: true,
            columns: [
              { key: "sl", label: "SL" },
          ...
          {renderTable({
            ...
            data: paymentsFromCustomers, useDateFilter: true,
            columns: [
              { key: "sl", label: "SL" },
